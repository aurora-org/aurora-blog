...
//		Scan(ctx, &v)
//
func (tq *ThemeQuery) Select(field string, fields ...string) *ThemeSelect {
...
func (tq *ThemeQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(theme.Columns...)...).From(t1)
	if tq.sql != nil {
		selector = tq.sql
		selector.Select(selector.Columns(theme.Columns...)...)
	}
...
func (ts *ThemeSelect) sqlQuery() sql.Querier {
	selector := ts.sql
